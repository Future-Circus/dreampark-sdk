    ...
    public class ParkSimOverlay : Overlay
    {
        ...
        private void Refresh()
        {
            ...
            _status.text = string.Format(
                "{0} placed · seed {1} · {2:F0}ms{3}",
                report.items.Count, report.seed, report.generateMilliseconds,
            ...
            // selection, so only when the contents actually changed.
        ...
        private void BuildList(ParkSimReport report, int dirty)
        {
            ...
            foreach (var item in report.items) {
                ...
                var label = new Label(item.fromSample ? item.name + "  (Sample)" : item.name);
