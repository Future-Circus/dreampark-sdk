...
{  
    ...
    public class ContentAPI
    {
        ...
        public static void UploadContent(string contentId, string releaseNotes, Action<bool, APIResponse> callback) {
            UploadContent(contentId, releaseNotes, null, callback);
        }
        ...
        #if UNITY_EDITOR
            Unity.EditorCoroutines.Editor.EditorCoroutineUtility.StartCoroutineOwnerless(UploadFlow(contentId, files, releaseNotes, schemaVersion, callback));
        #else
            CoroutineRunner.Run(UploadFlow(contentId, files, releaseNotes, schemaVersion, callback));
        #endif
        }
        ...
        private static IEnumerator UploadFlow(string contentId, List<KeyValuePair<string, UploadContentData>> files, string releaseNotes, int? schemaVersion, Action<bool, APIResponse> callback)
        {
            // Convert coroutine to async UniTask for concurrency
            UploadFlowAsync(contentId, files, releaseNotes, schemaVersion, callback).Forget();
            yield break;
        }

        private static async UniTaskVoid UploadFlowAsync(string contentId, List<KeyValuePair<string, UploadContentData>> files, string releaseNotes, int? schemaVersion, Action<bool, DreamParkAPI.APIResponse> callback)
        {
            int uploaded = 0;
            int failed = 0;
            int versionNumber = 1;
        ...
        private static async UniTask<(bool success, string uploadPath)> HandleFileUpload(string contentId, string platform, UploadContentData file)
        {
            try
            {
                ...
                MarkUploadComplete(platform, file.fileName, successUpload);
                if (successUpload)
