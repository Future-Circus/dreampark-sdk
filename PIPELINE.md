...
4. **Add Component → AttractionTemplate**. The `[RequireComponent]` will auto-add `GameArea` and `MusicArea`. Set: `size = Medium` (or whatever Phase 0 defined), `defaultAnchorPosition = (0, -3.2)` (where the player portal-spawns relative to the floor center, in meters), `generateFloor = true`, `floorMaterial = Assets/DreamPark/Materials/Occlusion.mat`. **Do not try to set `gameId`** — it's `[ReadOnly]` on `LevelTemplate`, `PropTemplate` and `PlayerRig`, and `ContentProcessor` stamps it from the content folder name. If this attraction depends on a real wall behind it — a portal window, a wall-mounted sign — choose every applicable edge in the `walls` multi-select dropdown (Front/Back/Right/Left in the attraction's own local frame; two adjacent sides describe a corner, two opposite sides describe a pass-through). This ships with the dimensions upload so layout and AI tools know to place it against a wall, along with the required wall height in feet (10 ft default, taller if your content reaches higher — the server falls back to 10 ft on its own if this isn't sent). A filled blue gizmo plane previews each selected side at the same height.
...
| Mark which sides need a real wall (portal, wall-mounted sign) | `LevelTemplate.walls` is a Front/Back/Right/Left multi-select; `PropTemplate.wallSide` chooses exactly one of those edges |
