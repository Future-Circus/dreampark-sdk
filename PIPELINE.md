...
4. **Add Component → AttractionTemplate**. The `[RequireComponent]` will auto-add `GameArea` and `MusicArea`. Set: `size = Medium` (or whatever Phase 0 defined), `defaultAnchorPosition = (0, -3.2)` (where the player portal-spawns relative to the floor center, in meters), `generateFloor = true`, `floorMaterial = Assets/DreamPark/Materials/Occlusion.mat`. **Do not try to set `gameId`** — it's `[ReadOnly]` on `LevelTemplate`, `PropTemplate` and `PlayerRig`, and `ContentProcessor` stamps it from the content folder name. If this attraction depends on a real wall behind it — a portal window, a wall-mounted sign — toggle the relevant side(s) on `walls` (any combination of `Forward`/`Back`/`Right`/`Left`, in the attraction's own local frame); this ships with the dimensions upload so layout and AI tools know to place it against a wall. A blue gizmo plane previews each toggled side at 10 ft tall (taller if your content is taller) when the attraction is selected.
...
| Mark which sides need a real wall (portal, wall-mounted sign) | `AttractionTemplate.walls` (any of `Forward`/`Back`/`Right`/`Left`) or `PropTemplate.wallSide` (`Right`/`Left`) |
